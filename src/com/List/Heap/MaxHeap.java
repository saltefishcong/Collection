...
public class MaxHeap<T extends Comparable> { // 最大堆
	...
	public void add(T data) {
		// 将要插入的元素放在数组的第一个不为null的位置上,然后循环和父节点对比,直到父节点比插入节点大
		if (isEmpty()) {
			arr[++size] = data;
			return;
		}
	...
	public T remove() { // 将最后一个节点放到第一个节点,然后和左右节点对比,直到左右节点比最后节点小为止
		...
		while (index < size) {
			...
			if (index != size && arr[index + 1].compareTo(arr[index]) > 0)
				// 判断左右节点那个大,找到大节点的位置
				index++;
			if (arr[index].compareTo(arr[new_index]) > 0) { // 判断左右节点是否比父节点大,大就交换
				d = arr[new_index];
				arr[new_index] = arr[index];
				arr[index] = d;
			} else {
				// 不比父节点大就退出,因为这是个最大堆,某个节点如果不比父节点大,那么其左右节点都不可能比父节点大,正常情况下
				break;
			}
		}
		arr[size] = null;
